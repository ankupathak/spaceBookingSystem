...
public class GlobalExceptionHandler {
    ...
    public ResponseEntity<ErrorResponse> handleLockFail(CannotAcquireLockException ex, HttpServletRequest request) {
        ErrorCode code = ErrorCode.UNEXPECTED;
        ErrorResponse.ErrorResponseBuilder buildError = ErrorResponse.builder();
        buildError
                .status(code.getStatus().value())
...
//        return problem(HttpStatus.CONFLICT, "booking-conflict",
//                "Booking Conflict",
//                "This time slot is no longer available. Please choose a different time.");
    ...
    public ResponseEntity<ErrorResponse> handleApp(
        ...
        includeDevMessage(buildError,ex.getDevMessage());
        buildResponse(buildError,request);
        return ResponseEntity.status(code.getStatus()).body(
                buildError.build()
        );
